...
#define BLOOM_INTENSITY _BloomParameter.w
#define BLOOM_THRESHOLD _BloomParameter.z
...
half4 fragHDRBase(v2f_hdr i) : SV_Target
{
	...
	half w = lerp(_ToneMappingParameter.y, center.r, 0.75) / (bloom.a + 1);
	//half w = center.r / (bloom.a + 1);
	color.rgb = ToneMapping(color.rgb * e, w);
	color.rgb += ToneMapping(bloom.rgb * e, w * max(1 / 32.0, 1 - BLOOM_INTENSITY));
